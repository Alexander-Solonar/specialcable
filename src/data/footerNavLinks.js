export const footerNavLinks = [
  {
    title: 'footer.sections.about.title',
    items: [
      [{ label: 'footer.sections.about.items.aboutUs', href: '/about-us' }],
      [{ label: 'footer.sections.about.items.news', href: '##' }],
      [{ label: 'footer.sections.about.items.gallery', href: '##' }],
      [{ label: 'footer.sections.about.items.vacancies', href: '##' }],
      [{ label: 'footer.sections.about.items.articles', href: '##' }],
      [{ label: 'footer.sections.about.items.partners', href: '##' }],
    ],
  },
  {
    title: 'footer.sections.catalog.title',
    items: [
      [
        { label: 'footer.sections.catalog.items.phoneAndBroadcastWires', href: '##' },
        { label: 'footer.sections.catalog.items.heatingCables', href: '##' },
      ],
      [
        { label: 'footer.sections.catalog.items.securityFireCables', href: '##' },
        { label: 'footer.sections.catalog.items.videoSurveillanceCables', href: '##' },
      ],
    ],
  },
  {
    title: 'footer.sections.information.title',
    items: [
      [
        { label: 'footer.sections.information.items.contractorRequirements', href: '##' },
        { label: 'footer.sections.information.items.paymentTerms', href: '##' },
        { label: 'footer.sections.information.items.certificates', href: '##' },
      ],
      [
        { label: 'footer.sections.information.items.storageConditions', href: '##' },
        { label: 'footer.sections.information.items.supplierInfo', href: '##' },
      ],
    ],
  },
  {
    title: 'footer.sections.contacts.title',
    items: [
      [
        { label: 'footer.sections.contacts.items.phone', href: '##' },
        { label: 'footer.sections.contacts.items.email', href: '##' },
        { label: 'footer.sections.contacts.items.warehouse', href: '##' },
      ],
      [
        { label: 'footer.sections.contacts.items.office', href: '##' },
        { label: 'footer.sections.contacts.items.factory', href: '##' },
      ],
    ],
  },
];
